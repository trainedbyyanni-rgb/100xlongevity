...
7. Form submissions are NOT yet forwarded to trainedbyyanni@gmail.com. There is no HTML/code-level way to set a Netlify Forms notification destination — it must be configured in the Netlify dashboard: Site settings → Forms → Form notifications → Add notification → Email notification → enter trainedbyyanni@gmail.com. Two separate forms now exist and each needs this set up independently, since Netlify treats each unique `name` attribute as its own form: `consultation-questionnaire` (coaching.html) and `d2-guide-optin` (guides/stimulant-not-hitting-anymore.html). This is a one-time manual setup step per form, not something a future coding session can do by editing files.
8. guides/ is a new directory for gated lead-magnet pages (email-required content unlocks), distinct from cognitive/ lesson pages. guides/stimulant-not-hitting-anymore.html is the first one — repurposes the D2 receptor recovery content from cognitive/d2-receptor-recovery.html in a punchier, opt-in-gated format. It is not linked from any nav or hub page; reaching it requires the direct URL.
